...
init(Args) ->
	...
	Announce  = <<"butt", 0:8, (size(Name)):8, Name/binary>>,
...
handle_info({udp, _Socket, _IP, 1970, Announce}, #{ announce := Announce } = State) ->
	{noreply, State};
handle_info({udp, _Socket, _IP, 1970, <<"butt", 0:8, Size:8, Name:Size/binary>>}, State) ->
	Node = erlang:binary_to_atom(Name, utf8),
	AlreadyConnected = lists:member(Node, nodes(connected)),
	if
		AlreadyConnected -> ok;
		not AlreadyConnected -> net_adm:ping(Node)
	end,
	{noreply, State};
